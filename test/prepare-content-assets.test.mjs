...
test("stages referenced Obsidian image embeds for routed pages", async () => {
  const siteRoot = await mkdtemp(path.join(tmpdir(), "word-pages-assets-"))
...
`)
  ...
  const stagedHome = await readFile(path.join(siteRoot, ".word-pages", "quartz-content", "index.md"), "utf8")
  assert.match(stagedHome, /!\[hero\]\(\.\/assets\/images\/hero\.png\)/)

  const stagedImage = await readFile(path.join(siteRoot, ".word-pages", "quartz-content", "assets", "images", "hero.png"), "utf8")
  assert.equal(stagedImage, "fake image")

  await assert.rejects(
    readFile(path.join(siteRoot, ".word-pages", "quartz-content", "assets", "images", "draft.png"), "utf8"),
    { code: "ENOENT" }
  )
})

test("uses route-relative paths for post image embeds", async () => {
  const siteRoot = await mkdtemp(path.join(tmpdir(), "word-pages-post-assets-"))
  await cp(path.join(repoRoot, "template", "scripts"), path.join(siteRoot, "scripts"), { recursive: true })
